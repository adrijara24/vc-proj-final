...
\begin{itemize}
    ...
    \item \textbf{Filtratge de candidats}: Kes regions extreten passen per un filtre final on es calculen paràmetres derivats per descartar falsos positius evidents abans de retornar els resultats.
\end{itemize}
...
\textbf{Criteris de filtratge gemoètric}
...
\begin{itemize}
    ...
    \item \textbf{isReasonable}: Es limita l'àrea relativa del blob. S'eliminen sorolls petits ($\lt 0.05\%$ i \textit{blobs} que ocupin més d'un 25\% de la imatge total. Assumim el risc teòric de descartar matrícules que ocupin tota la pantalla (poc comú en un entorn real) per evitar errors severs on es selecciona la carrosseria sencera del vehícle com a candidata (ex. \textit{test\_097}).
\end{itemize}
